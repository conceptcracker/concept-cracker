// ==================================================
// 🔥 100% ORIGINAL TCS iON CALCULATOR (PIXEL-PERFECT MECHANICAL 3D) 🔥
// ==================================================

// 1. EXACT CSS INJECTION (THICK BORDERS FOR REAL 3D FEEL, NO SQUEEZING)
...
calcStyle.innerHTML = `
    ...
    /* SUNKEN DISPLAY AREA (Dhasa hua look) */
    ...
    /* TOP CONTROLS ROW (MC, MR, mod - No Squeezing) */
    ...
    /* ---------------------------------------------------
       🔥 THE REAL MECHANICAL 3D BUTTON CSS 🔥
       --------------------------------------------------- */
    ...
    .tcs-btn { 
        ...
        border-bottom: 3px solid #a0a0a0; /* THIS IS THE REAL 3D SECRET */
        ...
        outline: none; 
        padding: 0;
        transition: none; /* No transition for snappy hardware feel */
    }
    
    /* FIX FOR TOP ROW BUTTONS (mod, MC, MR...) */
    .tcs-row1-btn {
        min-width: 46px; /* Prevents squeezing */
        padding: 0 5px;
    }
    ...
    .tcs-btn:active { 
        ...
        transform: translateY(2px); /* Moves down physically */
    }
    
    /* NUMBERS & BASIC MATH (Bigger font as per original) */
    ...
    .tcs-btn-red { 
        ...
        border-bottom: 3px solid #b33527 !important; 
        border-right: 1px solid #b33527 !important;
        font-size: 16px; 
    }
    .tcs-btn-red:active { 
        border-bottom: 1px solid #b33527 !important; 
    ...
    .tcs-btn-green { 
        ...
        border-bottom: 3px solid #219653 !important; 
        border-right: 1px solid #219653 !important;
        font-size: 22px !important; 
        grid-row: span 2; 
        height: 100% !important; 
    }
    .tcs-btn-green:active { 
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    calcDiv.innerHTML = `
        <div id="tcs-drag-handle">
            ...
            <div class="tcs-top-actions">
                ...
                <span class="tcs-win-btn">_</span>
                <span class="tcs-win-btn close-calc" onclick="tcsToggle()">✖</span>
            </div>
        </div>
        <div class="tcs-body">
            <div class="tcs-disp-box">
    ...
    // 📱 SMOOTH DRAG LOGIC (Desktop + Mobile)
    var cCon = document.getElementById("tcs-calc-container");
    var dHan = document.getElementById("tcs-drag-handle");
    var isD = false, oX, oY;

    const startDrag = (e) => {
        if(e.target.tagName.toLowerCase() === 'button' || e.target.classList.contains('close-calc')) return;
        isD = true;
...
// 3. MATHEMATICAL LOGIC & WORKING MEMORY
var tTop="", tBot="0", isN=true, pO=null, pV=null, mem=0;

function tD(){ 
    document.getElementById('tcs-val').value=tBot; 
    document.getElementById('tcs-exp').value=tTop; 
}
...
function tMem(op){
    if(op==='MC') { mem=0; }
    else if(op==='MR') { tBot=mem.toString(); isN=true; tD(); }
    else if(op==='MS') { mem=parseFloat(tBot); isN=true; }
    else if(op==='M+') { mem+=parseFloat(tBot); isN=true; }
    else if(op==='M-') { mem-=parseFloat(tBot); isN=true; }
}
...
function tB(op){
    if(pO && !isN) tBot = evB(pV, pO, tBot).toString();
    pV = tBot; pO = op;
    
    let opSym = op;
    if(op==='yroot') opSym = 'yroot';
    if(op==='logy') opSym = 'log base';
    if(op==='Exp') opSym = 'E';
...
function tEq(){
    if(pO){
        let opSym = pO;
        if(pO==='yroot') opSym = 'yroot';
        if(pO==='logy') opSym = 'log base';
        if(pO==='Exp') opSym = 'E';
        
        tTop = pV + " " + opSym + " " + tBot + " =";
        tBot = evB(pV, pO, tBot).toString();
        pO = null; isN = true; tD();
    }
}

function evB(v1, op, v2){
    v1=parseFloat(v1); v2=parseFloat(v2);
    if(op==='+') return v1+v2;
    if(op==='-') return v1-v2;
    if(op==='*') return v1*v2;
    if(op==='/') return v1/v2;
    if(op==='mod' || op==='%') return v1%v2;
    if(op==='x^y') return Math.pow(v1,v2);
    if(op==='yroot') return Math.pow(v1, 1/v2);
    if(op==='logy') return Math.log(v1)/Math.log(v2);
    if(op==='Exp') return v1 * Math.pow(10, v2);
    return v2;
}

function tClr(){ tTop=""; tBot="0"; pO=null; pV=null; isN=true; tD(); }
function tDel(){ if(!isN){ tBot=tBot.slice(0,-1)||"0"; tD(); } }
function tPi(){ tBot=Math.PI.toString(); isN=true; tD(); }
function tE(){ tBot=Math.E.toString(); isN=true; tD(); }
